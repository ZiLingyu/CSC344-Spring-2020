...
int main() {
    ...
    while (getline(&line, &length, file) != -1 && counter < numLines) {
        ...
        if (strcmp(tok, "LIT") == 0 || strcmp(tok, "DUP") == 0 || strcmp(tok, "IFEQ") == 0 ||
            strcmp(tok, "IFLT") == 0 || strcmp(tok, "JUMP") == 0 || strcmp(tok, "STOR") == 0 || strcmp(tok, "LOAD") == 0) {
            // Get second operation
            int operationData;
            if (strcmp(tok, "STOR") == 0 || strcmp(tok, "LOAD") == 0) {
                tok = strtok(NULL, " ");
                // Remove $ from beginning ($00 -> 00)
                tok[0] = tok[1];
                tok[1] = tok[2];
                tok[3] = '\0';
                operationData = strtol(tok,NULL,16);
            } else {
                tok = strtok(NULL, " ");
                operationData = atoi(tok);
            }
    ...
    int jumped = 0;
    int * memory = (int*) malloc(256*sizeof(int));
